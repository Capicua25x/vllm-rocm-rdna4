...
class Gemma4ForCausalLM(nn.Module, SupportsLoRA, SupportsPP, MixtureOfExperts, SupportsEagle, SupportsEagle3):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # Live EAGLE-1 training-data capture: only TP rank 0 writes. We check
        # the observability_config flag here and leave a fast Python-level
        # boolean on self so the per-forward check is a single attribute
        # load. The writer singleton is module-level so multiple Gemma4
        # instances (there is normally only one) share it.
        self._capture_hidden_enabled = False
        obs_cfg = getattr(vllm_config, "observability_config", None)
        capture_dir = getattr(obs_cfg, "capture_hidden_gemma4", None) if obs_cfg else None
        if capture_dir:
            if get_tensor_model_parallel_rank() == 0:
                # Delay import to avoid cycles / unnecessary cost when the
                # flag isn't used.
                from vllm.v1.worker.gemma4_hidden_capture import init_writer
                init_writer(capture_dir)
                self._capture_hidden_enabled = True
                logger.info(
                    "[gemma4-capture] Enabled live hidden-state capture to %s "
                    "(TP rank 0)",
                    capture_dir,
                )
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Note: EAGLE-1 live capture is done in
        # GPUModelRunner.execute_model (post-forward, pre-LM-head) because
        # the MM wrapper (Gemma4ForConditionalGeneration) passes
        # input_ids=None to this forward whenever inputs_embeds is used.
        return hidden_states
