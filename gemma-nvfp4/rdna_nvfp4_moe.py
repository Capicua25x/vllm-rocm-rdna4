...
class RdnaNvFp4ExpertsMonolithic(mk.FusedMoEExpertsMonolithic):
    ...
    def apply(
        ...
        # NOTE: the field on FusedMoEConfig is `routing_method` (not
        # `routing_method_type`) on both vLLM 0.18.1 and 0.19.1; the canonical
        # idiom (cf. TrtLlmNvFp4ExpertsMonolithic.__init__) is
        # `self.moe_config.routing_method`. Reading the wrong name returned None
        # and silently bypassed the Gemma-4 Custom branch below — defeating the
        # whole GAP-1 routing fix. Use the real field so Custom actually fires.
        routing_method_type = getattr(self.moe_config, "routing_method", None)
