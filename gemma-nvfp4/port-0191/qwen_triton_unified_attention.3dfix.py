...
# Spec-decode 6K fix (2026-06-25): the 3D split-KV (flash-decode) kernel is gated
# to pure decode (max_seqlen_q <= 1); the spec VERIFY/propose forward has query_len>1
# (bonus + drafted tokens) so it falls to the low-occupancy 2D kernel, which at low
# batch + long context has a tiny launch grid -> the 6K spec-decode regression. The
# 3D kernel is generic over q-blocks and its segm buffers are token-indexed with
# capacity seq_threshold_3D, so it's correct + safe for query_len>1 when total query
# tokens fit (q.shape[0] <= seq_threshold_3D). VLLM_TRITON_3D_SMALL_QLEN=1 gates the
# 2D fallback on token count instead of blanket-banning query_len>1.
...
def kernel_unified_attention_2d(
    ...
    lse_ptr=None,  # [num_tokens, num_query_heads] or None
    lse_stride_0: tl.int64 = 0,  # stride along token dim
    RETURN_LSE: tl.constexpr = False,  # bool
):
    ...
    for j in range(tile_start, tile_end):
        ...
        if CAUSAL:
            ...
            seq_mask = seq_offset[None, :] < max_seq_prefix_len
    ...
    # Write LSE = M + log(L) for PCP/DCP cross-rank attention merging.
    # Shape: [num_tokens, num_query_heads], same as reduce_segments output.
    if RETURN_LSE:
        lse_val = M + tl.log(tl.where(L > 0.0, L, 1.0))
        lse_offset = query_offset_0 * lse_stride_0 + query_offset_1
        tl.store(
            lse_ptr + lse_offset,
            lse_val,
...
def reduce_segments(
    ...
    # Store LSE for DCP cross-rank attention merging
    if RETURN_LSE:
        lse_val = overall_max + tl.log(
            tl.where(overall_expsum > 0.0, overall_expsum, 1.0)
        )
        lse_offset = query_token_idx * num_query_heads + query_head_idx
...
def unified_attention(
    ...
    # causal=False is used by the DCP context-attention pass where
    # all KV is strictly in the past so no causal mask is needed.
    ...
    if seq_threshold_3D is None:
        _use_2d = True
    elif _ALLOW_3D_SMALL_QLEN:
        _use_2d = q.shape[0] > seq_threshold_3D
    else:
    ...
    if (
        ...
        kernel_unified_attention_2d[
            ...
            CAUSAL=causal,
            lse_ptr=lse,
            lse_stride_0=num_query_heads if return_lse else 0,
