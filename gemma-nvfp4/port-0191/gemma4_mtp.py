...
class Gemma4MTPAttention(nn.Module):
    ...
    def __init__(
        ...
        self.attn = Attention(
            ...
            # Draft is dense bf16 (checkpoint has only q_proj/q_norm/o_proj, no
            # quant scales) and shares only the target's bf16 KV cache. Do NOT
            # inherit the target's compressed-tensors quant_config here, or the
            # attn creates q/k/v scale+zero_point params absent from the draft
            # checkpoint -> "weights were not initialized". (kv-cache-dtype=auto.)
            quant_config=None,
